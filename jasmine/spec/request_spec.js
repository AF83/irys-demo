...
describe("For the requests it", function() {
  var dataObject, dataObjectGM, dateIso, form, formGeneralMessage, formPrefixSiri13, formPrefixSiri20, formTest1, request, requestCheck, requestCheckGM, xmlRequest;
  dateIso = new Date;
  ...
  it("checks the form is parsed correctly for GeneralMessage when siri Version set to 2.0", function() {
    var expected, prop, results;
    request.getGeneralMessage(formGeneralMessage);
    results = [];
    for (prop in dataObjectGM) {
      expected = dataObjectGM[prop];
      results.push(expect(request[prop]).toEqual(expected));
    }
    return results;
  });
  ...
  it("checks the GeneralMessage request is right when siri Version set to 2.0", function() {
    var expected, gmRequest, prop, results;
    gmRequest = request.getGeneralMessage(formGeneralMessage);
    results = [];
    for (prop in requestCheckGM) {
      expected = requestCheckGM[prop];
      results.push(expect($.parseXML(gmRequest).getElementsByTagName(prop)[0].innerHTML).toEqual(expected));
    }
    return results;
  });
